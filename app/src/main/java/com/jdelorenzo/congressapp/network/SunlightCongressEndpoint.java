...
public interface SunlightCongressEndpoint {
  ...
  Call<LegislatorResult> getLegislatorsByLatLong(
  ...
  Call<List<District>> getDistrictsByLatLong(
      @Query("latitude") float latitude, @Query("longitude") float longitude);
  ...
  @GET("committees")
  Call<List<Legislator>> getCommitteeMembers(
      @Query("committee") String committee, @Query("fields") String members);
  ...
  @GET("bills/search")
  Call<List<Bill>> searchLaws(
      @Query("query") String search, @Query("history.enacted") Boolean enacted);
}
