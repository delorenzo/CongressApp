 ...
 */
@Database(entities = {Legislator.class}, version=1)
    ...
    public static LegislatorsDatabase getInstance(Context context) {
        if (INSTANCE == null) {
            synchronized (LegislatorsDatabase.class) {
                if (INSTANCE == null) {
                    INSTANCE = Room.databaseBuilder(context.getApplicationContext(), LegislatorsDatabase.class, LEGISLATOR_DATABASE_NAME).build();
                }
            }
        }
        return INSTANCE;
